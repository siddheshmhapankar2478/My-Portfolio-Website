...
const CustomLink = ({href, title, className=""}) => { 
  ...
  return(
  <Link href={href} className={`${className} relative group text-dark ` }> {title}
  ...
  return (
    <header className='min-w-full w-full z-10 shadow-sm px-32 py-5 font-bold text-lg flex items-center  gap-5 sticky top-0 left-0 right-0 bg-light dark:bg-dark dark:text-light lg:px-0 md:px-0 sm:px-0 xs:px-0'>
        ...
        (<motion.div 
         ...
         className="min-w-[70vw] hidden lg:flex md:flex sm:flex flex-col justify-between items-center fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 text-light dark:text-dark bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32">
